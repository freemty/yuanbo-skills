...
读取目标仓库的提交与产物；只有可独立扫描且宿主支持时才委派，否则主线程完成：
...
检查实际子模块和相关产物；无 commit 不等于无进展，已提供的实验记录也可作为证据。
...
根据现有范围交付完整周报。复用用户已提供的下周计划和协作项，不编造承诺；缺失时注明未提供。用户希望迭代时，再按反馈调整项目取舍与粒度，不把再次确认作为完成前提。
