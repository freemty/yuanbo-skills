...
- **LabMate**: replace reminder-heavy orchestration with three focused hooks and deterministic plan/apply/check or CRUD scripts for project setup, experiments, TODOs, and snapshots.

### 构建与工具链
- **Context audit**: add a repository-wide policy check for entrypoint size, imperative density, host leakage, missing references, and duplicate project-skill mirrors; CI now requires a `50 skills; 0 flagged` result.
